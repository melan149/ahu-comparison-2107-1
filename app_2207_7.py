...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    if chart_data_area:
        ...
        # Ensure Unit Size is treated as string for text label
    ...
    for col in df.columns:
        ...
        # If a header is triggered AND it's "Certification data", insert the chart here
        if triggered_header == "Certification data":
            # This is where the new chart should be inserted, before the "Certification data" header
            # The chart drawing logic has been moved up, so we just need to print the header.
            st.markdown("---")
            ...
            with col3:
                st.markdown(f'<div style="text-align: center;">**{selected_brand2} - {selected_unit2} - {selected_size2}**</div>', unsafe_allow_html=True)
            # Continue to the next iteration to display the parameter that triggered this header
            # (e.g., "Unit size quantity" will be displayed in the next iteration if not excluded)
            # We don't need to explicitly set chart1_displayed, chart2_displayed, etc. here as they are handled
            # by their respective column triggers.
