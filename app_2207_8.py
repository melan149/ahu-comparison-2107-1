...
header_triggers_raw = {
    unit_size_quantity_col: "Certification data", # Use the resolved column name for trigger
...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    # unit_area_col_name is used in the new chart, so it should be excluded from the main table
    unit_area_col_name = get_column_safe(df, ["Unit cross section area (Supply Filter) [m2]"])
    if unit_area_col_name: raw_excluded_cols.append(unit_area_col_name) 
    ...
    for col in df.columns:
        ...
        # --- Insert NEW CHART: Unit Cross Section Area (Supply Filter) vs. Unit Sizes ---
        # This chart should be placed AFTER "Unit size quantity" row and BEFORE "Certification data" header
        if col == unit_size_quantity_col and not unit_area_chart_displayed: # Trigger for the new chart
