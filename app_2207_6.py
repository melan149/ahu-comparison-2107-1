...
# Updated column names based on user's latest input
...
sens_efficiency_nominal_pcr_hex_col = get_column_safe(df, ["Sens. efficiency at nominal balanced airflows_PCR/HEX [%]", "Sens. efficiency at nominal balanced airflows [%].1"]) # Assuming .1 for the second instance if it's the same base name
...
air_speed_filter_max_airflow_col = get_column_safe(df, ["Air speed on Filter at max airflow (ErP) [m/s]", "Air speed on Filter at max airflow (ErP)"])
...
header_triggers_raw = {
    ...
    # Updated triggers based on new header list
    metal_sheet_thickness_external_col: "Airflows",
    air_speed_filter_max_airflow_col: "Dimensions",
    duct_connection_diameter_col: "Rotary wheel",
    sens_efficiency_nominal_pcr_hex_col: "PCR/HEX recovery exchanger", # Trigger for PCR/HEX section
    impeller_efficiency_col: "Fan section data", # Trigger for Fan section
    heating_elements_type_col: "Electrical heater", # Adjusted trigger for Electrical Heater
    ...
    final_pd_supply_col: "Exhaust Filter", # Updated trigger name
...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    # --- NEW CHART: Unit Cross Section Area (Supply Filter) vs. Unit Sizes ---
    ...
    if unit_area_col_name and unit_area_col_name in df.columns and size_col in df.columns:
        ...
        # Filter for left selection, up to recovery type, to get all unit sizes
        df_for_chart1 = df[
        ...
        # Apply conditional filtering for type/material if applicable
        if selected_recovery1 == "RRG" and type_col and selected_type1:
            df_for_chart1 = df_for_chart1[df_for_chart1[type_col] == selected_type1]
        elif selected_recovery1 in ["HEX", "PCR"] and material_col and selected_material1:
            df_for_chart1 = df_for_chart1[df_for_chart1[material_col] == selected_material1]

        # Filter for right selection, up to recovery type, to get all unit sizes
        df_for_chart2 = df[
        ...
        if selected_recovery2 == "RRG" and type_col and selected_type2:
            df_for_chart2 = df_for_chart2[df_for_chart2[type_col] == selected_type2]
        elif selected_recovery2 in ["HEX", "PCR"] and material_col and selected_material2:
            df_for_chart2 = df_for_chart2[df_for_chart2[material_col] == selected_material2]

        # Collect data for the chart from both filtered DataFrames
        if not df_for_chart1.empty and unit_area_col_name in df_for_chart1.columns and size_col in df_for_chart1.columns:
            for index, row in df_for_chart1.iterrows():
        ...
        if not df_for_chart2.empty and unit_area_col_name in df_for_chart2.columns and size_col in df_for_chart2.columns:
            for index, row in df_for_chart2.iterrows():
                if pd.notna(row[unit_area_col_name]) and pd.notna(row[size_col]):
                    chart_data_area.append({
                        "Unit Size": row[size_col],
                        "Unit Cross Section Area (m²)": row[unit_area_col_name],
    ...
    if chart_data_area:
        ...
        # Ensure Unit Size is treated as categorical for correct plotting
    ...
    raw_excluded_cols = [
        ...
        sens_efficiency_nominal_rrg_col, sens_efficiency_opt_rrg_col, # Exclude RRG specific efficiencies
        sens_efficiency_nominal_pcr_hex_col, sens_efficiency_opt_pcr_hex_col # Exclude PCR/HEX specific efficiencies
    ]
    ...
    for x_name, y_name in coord_col_pairs_1_5:
        raw_excluded_cols.append(x_name)
        raw_excluded_cols.append(y_name)
    ...
    if capacity_range1_col: raw_excluded_cols.append(capacity_range1_col)
    if capacity_range2_col: raw_excluded_cols.append(capacity_range2_col)
    if capacity_range3_col: raw_excluded_cols.append(capacity_range3_col)
    if unit_area_col_name: raw_excluded_cols.append(unit_area_col_name) # Exclude new chart column from table

    # Filter out any None values from the excluded columns list
    excluded_cols_from_table = [col_name for col_name in raw_excluded_cols if col_name is not None]
    ...
    for col in df.columns:
        ...
        for raw_trigger_col_name, header_title in header_triggers_raw.items():
            # Use the already resolved column name from the dictionary
        ...
        if triggered_header:
            ...
            csv_data.append([triggered_header, "", ""]) # Add new section header

        # Add the parameter row if not excluded
    ...
    current_header_for_display = "General data" # Initial header for display

    # Loop for displaying the comparison table and charts
    for col in df.columns:
